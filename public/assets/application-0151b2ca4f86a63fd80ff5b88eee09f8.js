...
(function() {
  ...
  Carousel = (function() {
    function Carousel(element, url, orientation, options) {
      this.element = element;
      this.url = url;
      ...
      this.element.addClass("carousel-" + this.orientation);
      this.data = [];
      this.refresh();
      this.setScroll();
    }
    ...
    Carousel.prototype.refresh = function() {
      ...
      return $.ajax(this.url, {
        dataType: 'json',
        error: function(xhr, status, error) {
          return _this.setError("Couldn't load photos: " + (status === 'error' ? error : status) + ".");
        },
    ...
    Carousel.prototype.populate = function(data) {
      var _this = this;
      ...
      $.each(data, function(_, photo) {
        var a, fig, li;
        li = $('<li/>').appendTo(_this.element);
        ...
        if (_this.options['captions'] && photo.caption) {
          return $('<figcaption/>').text(photo.caption).appendTo(fig);
        }
      });
      if (this.options['lightboxes']) {
        this.element.find('a').lightBox({
      ...
      if (this.noScroll() && data.length > 0) {
        return this.loadNextPage();
      }
    };

    Carousel.prototype.setError = function(error) {
      var li;
      this.clear();
      li = $('<li/>').addClass('center').appendTo(this.element);
      return $('<p/>').addClass('error').text(error).appendTo(li);
    };

    Carousel.prototype.setNote = function(note) {
      var li;
      this.clear();
      li = $('<li/>').addClass('center').appendTo(this.element);
      return $('<p/>').addClass('note').text(note).appendTo(li);
    };

    Carousel.prototype.setScroll = function() {
      var _this = this;
      return this.element.scroll(function() {
        if (_this.isAtEnd()) {
    ...
    Carousel.prototype.noScroll = function() {
      switch (this.orientation) {
        case 'horizontal':
          return this.element.width() === this.element[0].scrollWidth;
        case 'vertical':
    ...
    Carousel.prototype.isAtEnd = function() {
      switch (this.orientation) {
        case 'horizontal':
          return this.element.scrollLeft() === this.element[0].scrollWidth - this.element.width();
        case 'vertical':
          return this.element.scrollTop() === this.element[0].scrollheight - this.element.height();
      }
    };

    Carousel.prototype.loadNextPage = function() {
      var _this = this;
      if (this.data.length === 0) {
        return;
      }
      return $.ajax(this.url, {
        data: {
          last_record: this.data[this.data.length - 1].id
        },
        dataType: 'json',
        error: function(xhr, status, error) {
          return _this.setError("Couldn't load photos: " + (status === 'error' ? error : status) + ".");
  ...
  $.fn.extend({
    carousel: function(url, orient, options) {
      if (options == null) {
        options = {};
      }
      return new Carousel(this, url, orient, options);
    }
  });
